...
vec4 render(in vec3 ro, in vec3 rd, float time) {
  ...
  for (int i = 0; i < 130; i++) {
    ...
    if (mpv.x > 0.6) {
      ...
      col = vec4(
          sin(vec3(5., 0.4, 0.2) + mpv.y * 0.1 + sin(pos.z * 0.4) * 0.5 + 1.8) *
                  0.5 +
              0.5,
          0.08);
      col *= den * den * den;
      ...
      col.xyz *=
          den * (vec3(0.005, .045, .075) + 1.5 * vec3(0.033, 0.07, 0.03) * dif);
    }
...
vec3 iLerp(in vec3 a, in vec3 b, in float x) {
  ...
  vec3 dir = normalize(vec3(2. * ic.x - ic.y - ic.z, 2. * ic.y - ic.x - ic.z,
                            2. * ic.z - ic.y - ic.x));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  col *= pow(16.0 * q.x * q.y * (1.0 - q.x) * (1.0 - q.y), 0.12) * 0.7 +
         0.3; // Vign
