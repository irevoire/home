...
function emoji_prompt() {
	...
	if [[ $TODAY = $BIRTHDAY ]]; then
		EMOJI="🎂" # Birthday!
	elif [[ $TODAY = 1231 ]]; then
		EMOJI="🍾" # New Years Eve
	elif [[ $TODAY = 0101 ]]; then
		EMOJI="🎊" # New Years Day
	elif [[ $TODAY = 0214 ]]; then
		EMOJI="💘" # Valentines Day
	elif [[ $TODAY = 1031 ]]; then
		EMOJI="🎃" # Halloween
	elif [[ $TODAY = 1231 ]]; then
		EMOJI="🎄" # Christmas
	else
...
function icat() {
	if [ "$TERM_PROGRAM" == "iTerm.app" ]; then
		imgcat "$@"
	elif [ "$TERMINFO" =~ .*"kitty".* ]; then
		kitty icat --align=left "$@"
	elif command -v viu &>/dev/null; then
		if [ -z "$@" ]; then # if there is no arguments
...
function check_recipes() {
	...
	for package in ${packages[@]}; do
		...
		echo "Checking package $package in path $package_path"
		cd $package_path
		ent check updates
	done
...
[ -s "$HOME/repos/aerynos/recipes/tools/helpers.zsh" ] && \. "$HOME/repos/aerynos/recipes/tools/helpers.zsh" # loads completions for aerynos
